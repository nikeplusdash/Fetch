...
One `docker compose up -d`, three containers — Jackett or Prowlarr (pick one,
Fetch talks to both the same way) plus FlareSolverr for the trackers that
gate themselves behind Cloudflare:
...
services:
  ...
  # Point Jackett's or Prowlarr's indexer settings at this for
  # Cloudflare-gated trackers (rutracker, for instance).
  flaresolverr:
    image: ghcr.io/flaresolverr/flaresolverr:latest
    restart: unless-stopped
    environment: [TZ=Etc/UTC, LOG_LEVEL=info, CAPTCHA_SOLVER=none]
    ports: ["8191:8191"]
```

Open `http://<host>:9117` (Jackett) or `http://<host>:9696` (Prowlarr), add
your trackers, and copy the API key off the dashboard — that key and this
URL are what the fields above ask for.

**FlareSolverr's cookie is bound to the IP it was issued to.** A native
Jackett/Prowlarr resolving the tracker over IPv6 and a Dockerised
FlareSolverr resolving it over IPv4 get handed two different cookies, and
whichever one solved it last silently invalidates the other's — the
symptom is a challenge that keeps reappearing. Keep both on the same
address family.
