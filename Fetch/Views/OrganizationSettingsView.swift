...
struct OrganizationSettingsView: View {
    ...
    var body: some View {
        ...
        VStack(spacing: 0) {
            ...
            SettingsGroup(title: "Playback") {
                SettingRow(
                    label: "Play cloud items with",
                    help: model.installedPlayers.isEmpty
                        ? "Install IINA, VLC, mpv or QuickTime to stream from your "
                            + "debrid account without downloading."
                        : "Which player a double-click on a cloud item opens."
                ) {
                    Picker("", selection: playerBinding) {
                        Text("First available").tag(ExternalPlayer?.none)
                        ForEach(model.installedPlayers, id: \.self) { player in
                            Text(player.displayName).tag(ExternalPlayer?.some(player))
                        }
                    }
                    .labelsHidden()
                    .fixedSize()
                    .disabled(model.installedPlayers.isEmpty)
                }
            }
    ...
    private var playerBinding: Binding<ExternalPlayer?> {
        Binding(
            get: {
                UserDefaults.standard.string(forKey: DefaultPlayer.defaultsKey)
                    .flatMap(ExternalPlayer.init(rawValue:))
            },
            set: { model.setDefaultPlayer($0) })
    }
