...
final class AppModel {
    ...
    var settingsTab: SettingsTab = .debrid
    ...
    /// Grouped by content, or one row per release (§12.1's toolbar toggle).
    var isGrouped = true {
        didSet { UserDefaults.standard.set(isGrouped, forKey: Self.groupedDefaultsKey) }
    }

    /// Which category the next search asks for. Persisted: the pill someone
    /// left selected is the one they meant.
    var searchCategory: SearchCategory = .all {
        didSet {
            UserDefaults.standard.set(searchCategory.rawValue, forKey: Self.categoryKey)
        }
    }
    private static let categoryKey = "app.fetch.searchCategory"
    ...
    var safeSearch = true {
        didSet { UserDefaults.standard.set(safeSearch, forKey: Self.safeSearchKey) }
    }
    ...
    private static let groupedDefaultsKey = "app.fetch.searchGrouped"
    ...
    /// Display name for a `SearchResult.sources` entry — "Prowlarr · 1337x".
    func indexerName(for id: SearchProviderID) -> String? {
        for server in indexerServers {
            if let indexer = server.indexers.first(where: { $0.id == id }) {
                return server.indexers.count > 1
    ...
    init(credentialStore: any CredentialStore = FileCredentialStore()) {
        ...
        if UserDefaults.standard.object(forKey: Self.groupedDefaultsKey) != nil {
            self.isGrouped = UserDefaults.standard.bool(forKey: Self.groupedDefaultsKey)
        }
        if let raw = UserDefaults.standard.string(forKey: Self.categoryKey),
           let category = SearchCategory(rawValue: raw) {
    ...
    func pause(_ id: DownloadID) {
        guard let engine = engineForDownload[id] ?? engine else {
            ...
            report("This download can no longer be paused — the debrid provider "
                   + "it was using is no longer configured. Remove the row and add it again.")
            return
        }
    ...
    func resume(_ id: DownloadID) {
        guard let engine = engineForDownload[id] ?? engine else {
            ...
            // no engine owns it any more. Silently doing nothing left a button
            // that visibly did nothing.
            report("This download can no longer be resumed — the debrid provider "
                   + "it was using is no longer configured. Remove the row and add it again.")
            return
        }
    ...
    private func cancel(_ id: DownloadID) {
        guard let engine = engineForDownload[id] ?? engine else {
            ...
            // no engine owns it any more. Silently doing nothing left a button
            // that visibly did nothing.
            report("This download can no longer be cancelled — the debrid provider "
                   + "it was using is no longer configured. Remove the row and add it again.")
            return
        }
    ...
    func runSearch(_ text: String) async {
        ...
        let providers = configuredSearchProviders()
        guard !providers.isEmpty else {
            resetSearch(to: .noProviders)
            return
        }
        ...
        let aggregator = SearchAggregator(
            providers: providers, profile: qualityProfile, excludeAdult: safeSearch)
    ...
    /// Torznab.
    private func configuredSearchProviders() -> [any SearchProvider] {
        ...
        built.append(contentsOf: torznabProviders())
        return built
    }

    private func torznabProviders() -> [any SearchProvider] {
        activeIndexers.compactMap { server, indexer in
