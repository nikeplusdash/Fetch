...
enum TokenTables {
    ...
    /// Tokens that only appear on a PC game release.
    ///
    /// **A year is not a movie.** The kind fell through to
    /// `yearMatch != nil ? .movie : .other`, and a game repack carries a year
    /// as reliably as a film does — so "Red Dead Redemption 2: Ultimate Edition
    /// [Build 1491.50 + DLC's] (2019) PC | RePack от FitGirl" was typed Movie,
    /// under a Games pill, which is where it was noticed.
    ///
    /// Deliberately narrow. Every entry here is a repacker's name, a store, or
    /// a word that means nothing in film or television — none of them can
    /// appear in a legitimate movie release, which is the only property that
    /// makes checking them before the year safe. Generic words a film could
    /// carry (PC, Portable, Crack, Multi) are **not** here: "MULTi" is a
    /// language tag on plenty of European film releases, and a false Game is as
    /// wrong as the false Movie this replaces.
    static let gameSignal: [QualityTokenEntry<Bool>] = [
        QualityTokenEntry(true, aliases: [
            // Repackers, none of which is a word.
            "FITGIRL", "DODI", "ELAMIGOS", "XATAB", "KAOSKREW",
            // Scene groups that only ever release games, and that no film
            // could be called.
            "RAZOR1911", "SKIDROW", "TENOKE",
            // Stores and shapes of a game release.
            "GOG", "DENUVO", "GAMERIP",
        ]),
    ]
