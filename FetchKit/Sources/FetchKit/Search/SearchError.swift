...
public enum SearchError: Error, Sendable {
    ...
    /// The fan-out ended without this indexer reporting either way — not a
    /// timeout, which is a reported outcome, but a missing answer. Synthesised
    /// by `StreamedResultAccumulator` rather than thrown, so that a search can
    /// never finish with an indexer left unaccounted for.
    case neverAnswered
...
extension SearchError: CustomStringConvertible {
    public var description: String {
        ...
        case .neverAnswered:          "neverAnswered"
...
extension SearchError: LocalizedError {
    public var errorDescription: String? {
        ...
        case .neverAnswered:
            "The search finished without this indexer reporting either way. "
            + "Search again; if it keeps happening, test it in Settings → Search."
