...
enum TorznabCapsParser {
    ...
    static func parse(_ data: Data) throws -> ProviderCapabilities {
        ...
        // Checked before the result is built, not after: a rejected key answers
        // 200 with a well-formed `<error>` document, which parses cleanly into
        // capabilities with nothing in them. Endpoint resolution then saved the
        // server and called it connected. See `TorznabErrorDocument`.
