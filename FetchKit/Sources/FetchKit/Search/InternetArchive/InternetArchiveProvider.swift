...
public struct InternetArchiveProvider: SearchProvider {
    ...
    static func mediatype(for categories: [TorznabCategory]) -> String? {
        ...
        // **5070 *alone* is the Anime pill**, and it deliberately yields
        // nothing: its one plausible IA mediatype is `movies`, which would
        // return live-action films under a pill asking for anime.
        //
        // Testing for 5070's presence was enough only while Anime was the sole
        // pill that named it. TV now takes its whole tree, 5070 included — and
        // there the anime subcategory is one shelf among ten, not the request.
        // Reading it as the Anime pill sent TV searches to the Archive with no
        // mediatype constraint at all.
        let television = categories.map(\.id).filter { (5000..<6000).contains($0) }
        if television == [5070] { return nil }
