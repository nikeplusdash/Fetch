...
public actor CacheStatusStore {
    ...
    /// Per-provider tallies since the last drain.
    ///
    /// **Counted here because here is where the answer is.** Every other layer
    /// sees the *merged* badge — `DebridRouter.mergeCacheStates` folds three
    /// services into one state — so by the time a hash reaches the results
    /// table there is no longer any record of which service had it. Which is
    /// exactly the question worth asking of a subscription.
    ///
    /// Only providers that gave a definite answer are tallied. One that threw
    /// contributes an error, not a miss: a service that was unreachable did not
    /// fail to hold the torrent.
        ...
        await withTaskGroup(
            ...
            for await (id, outcome) in group {
                ...
                case .failure(let error):
                    var tally = hitStats[id] ?? DebridCacheStats()
                    // One error for the call, not one per hash: a service being
                    // unreachable is a single event, and counting it fifty
                    // times because fifty hashes were in flight would make one
                    // blip look like a broken service.
                    tally.recordError()
                    hitStats[id] = tally
