...
public enum CacheReadiness: Sendable, Equatable {
    ...
    /// **One sentence, because it is shown in a pop-up outside the window.**
    ///
    /// These were three sentences each, written when this was a banner inside
    /// the layout with room to explain itself. The panel that replaced the
    /// banner holds one sentence and at most one action, and the reason is in
    /// the rule rather than in the size of the box: anything needing a
    /// paragraph is a state rather than an event, and a state belongs on the
    /// screen it describes. What these were spending their extra sentences on
    /// is already said elsewhere — `showsCacheBadges` removes the column
    /// rather than filling it with question marks, so the absence is visible
    /// without being narrated, and the Settings action carries the fix.
    ///
    /// The first also named TorBox, which is one of three services and not
    /// necessarily the one the reader has.
    ...
    /// What the alert's one action says, or nil when there is nothing to do.
    ///
    /// **Here rather than at the call site, so it stays testable.** The message
    /// used to name Settings in its own prose, because it was a banner with no
    /// button and that was the only way to say where the fix lived. Now the
    /// panel carries an action and the prose does not repeat it — but "explains
    /// itself *and* names the way out" is the guarantee worth keeping, and if
    /// the title lived only in the SwiftUI call site it would be in the one
    /// target with no test bundle.
