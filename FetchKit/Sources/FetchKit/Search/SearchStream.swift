...
public enum SearchEvent: Sendable {
    ...
    ///
    /// **A count cannot be reconciled.** "7 indexers" tells the accumulator how
    /// many answers to expect and nothing about *which*, so an indexer that
    /// never reported — a dropped event, a stream torn down mid-fan-out — left
    /// the progress readout one short with no way to say who was missing, and
    /// no way to close the search out honestly. With the roster in hand,
    /// `.finished` can name whoever never came back and record it as the
    /// failure it is.
...
public struct StreamedResultAccumulator: Sendable {
    ...
    /// Who was asked this round, and who has answered.
    ///
    /// Per round rather than cumulative, because paging reuses the same
    /// accumulator: on page 2 every provider already owns a bucket from page 1,
    /// so "has a bucket" is not the same question as "has answered *this*
    /// request". Both reset on `.started`.
    ...
    public mutating func apply(_ event: SearchEvent, appending: Bool = false) -> [String] {
        ...
        case .finished:
            // **Every indexer gets a resolution before the search is over.**
            // The fan-out is supposed to deliver exactly one event per asked
            // provider, but "supposed to" is not a thing the progress readout
            // can check: if an event is dropped — a stream torn down while the
            // group is still draining — the run ends at "6 of 7" with the
            // seventh unaccounted for, and the bar clears anyway.
            //
            // Silence is not success. Anyone still missing is recorded as
            // having never answered, which counts toward `completed` so the bar
            // is honestly full, names them in the failure detail, and lets
            // "1 of 7 indexers failed" say what happened instead of the run
            // quietly being one indexer short.
