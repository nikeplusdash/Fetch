...
public enum DownloadGrouping {
    ...
    public static func section(for states: [DownloadState]) -> DownloadSection? {
        ...
        /**
         An all-`.onCloud` group is ready and belongs in Completed (the
         Library); one still holding a `.cloudQueued` placeholder belongs
         with the queue (Downloads). A group mixing a cloud state with a
         real one never reaches here — the checks above handle it.
         */
        if states.allSatisfy(\.isCloudOnly) {
            return states.contains(.cloudQueued) ? .queued : .completed
        }
        return .completed
    }
