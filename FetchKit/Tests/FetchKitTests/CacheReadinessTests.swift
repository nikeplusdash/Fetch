...
@Suite struct CacheReadinessTests {
    ...
    /// The way out used to have to be in the sentence, because the banner had
    /// no button. The panel that replaced it has one, and the sentence is held
    /// to one clause — so the guarantee moved from the prose to the pair, and
    /// this asserts the pair rather than the word.
    ...
    /// Nothing the reader can do about this one, so offering a control would
    /// be a button that leads to a pane where the problem is not.
