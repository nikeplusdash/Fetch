...
@Suite struct SearchParticipationTests {
    ...
    @Test func startedCountsOnlyParticipants() async {
        ...
        // The roster, not just the size of it: `.finished` reconciles against
        // these ids, so naming a provider that was never asked would invent a
        // failure for an indexer nobody queried.
